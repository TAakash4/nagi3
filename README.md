...
TypeScript、Express、PostgreSQL（Drizzle）、OpenAI API（`gpt-4o-mini`）で動く Telegram Bot です。会話から抽出した長期記憶は直接保存せず、Telegram 上で承認された候補だけを保存します。
...
Railwayのサービスには `DATABASE_URL`、`TELEGRAM_BOT_TOKEN`、`LLM_API_KEY`、`LLM_MODEL`を設定してください。`LLM_MODEL`が空の場合は`gpt-4o-mini`を使います。`PORT`はRailwayが自動設定します。`railway.json`に起動コマンド、`/health`のヘルスチェック、失敗時の再起動方針を定義しています。

既存環境との互換性のため、`LLM_API_KEY`がない場合に限り`OPENAI_API_KEY`も読み込みます。Railwayでは画像の設定どおり`LLM_API_KEY`を使えば変更不要です。
